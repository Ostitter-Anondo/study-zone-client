...
import useCountApproved from "../../utils/hooks/useCountApproved";
import { Link } from "react-router";

const AllSessions = () => {
	const [count] = useCountApproved();
	const pageSize = 6;
			...
			<div className="w-full h-screen flex flex-col justify-between">
				<div className="my-12 w-11/12 mx-auto grid md:grid-cols-2 lg:grid-cols-3 place-items-center gap-6">
					{sessions.map((session) => (
						<div key={session._id} className="card flex flex-col gap-6 bg-base-300 p-6 items-center max-w-72 text-center">
							...
							<Link className="btn btn-info">Details</Link>
						</div>
					))}
				</div>
				<div className="join mx-auto my-12">
					{[...Array(Math.ceil(count.count / pageSize)).keys()].map((item) => (
						<button
							key={item}
							onClick={async () => {
								await setPage(item);
